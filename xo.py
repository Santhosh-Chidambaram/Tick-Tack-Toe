...
#function for displaying the elements in the board
...
#function for selecting player selection
def select_player():
    
    choice = int(input("click 1 to be 'X' or -1 to be 'O'"))
    if choice == 1:
        return (1)
    else :
        return (-1)
...
#check that the board is full or not
def full_board_check(board):
    return ' ' not in board[1:]

#getting the choices for position
...
#All calling function
while True:
    
    print('Welcome to Tic Tac Toe!')
    
    toggle = select_player()
